<script lang="ts">
	...
	let { data } = $props();
	const { years, year, sort, dir, stats, totals } = $derived(data);

	function buildUrl(overrides: { year?: number; sort?: string; dir?: string }): string {
		const merged = { year, sort, dir, ...overrides };
...
{:else}
	...
	<p class="mt-3 text-sm text-gray-600 dark:text-gray-400">
		...
		<strong>{totals.runden}</strong> Spieler-Runden gesamt im Jahr {year}
		<span class="text-xs">(Summe der pro Spieler erfassten Runden)</span>.
	</p>

	{#if stats.length === 0}
		<p class="mt-4 text-sm text-gray-500">Keine Spieler haben in {year} gespielt.</p>
	{:else}
